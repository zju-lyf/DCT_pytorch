...
def CS(conv, conv2):
    b, c, h, w = conv.size()
    #print(conv.shape)
    cat_all = torch.cat((conv, conv2), 1)
    #print(cat_all.shape)
    cat_reshape = torch.reshape(cat_all, (b, 2, c, h, w))
    #print(cat_reshape.shape)
    cat_transpose = cat_reshape.transpose(1, 2).contiguous()
    #print(cat_transpose.shape)
    cat_view = cat_transpose.view(b, -1, h, w)
    #print(cat_view.shape)
    conv2 = cat_view[:, :c, :, :]
    conv2_2 = cat_view[:, c:, :, :]
    return conv2, conv2_2
...
class Img_DCT_BlockFusion(nn.Module):
    def __init__(self, num_classes=2, pretrained=False, fusion=('fc')):
        ...
        model1 = xception(num_classes=2, fusion=fusion)
    ...
    def forward(self, input):
        ...
        img_7_1,_ = CS(img_7, idct_7)
        _,dct_7_1 = CS(dct_img_7, dct_7)
        img_12_input = (img_7_1, 12)
        img_12 = self.backbone_img(img_12_input)
        dct_12_input = (dct_7_1, 12)
        dct_12 = self.backbone_dct(dct_12_input)
        idct_12 = DCT.idct(dct_12)
        #dct_img_12 = DCT.dct(img_12)
        img_12_1, _ = CS(img_12, idct_12)
        img_0_input = (img_12_1, 0)
