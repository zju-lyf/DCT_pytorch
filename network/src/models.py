...
def build_svm(input_shape, classes, l_1):
    inputs = keras.Input(shape=input_shape)
    x = layers.Flatten()(inputs)

    if classes > 1:
        outputs = 2 * layers.Dense(classes, activation="softmax",
                                   kernel_regularizer=keras.regularizers.l1(l_1))(2*x) - 1
    else:
        outputs = layers.Dense(classes, activation="tanh",
                               kernel_regularizer=keras.regularizers.l1(l_1))(x)
